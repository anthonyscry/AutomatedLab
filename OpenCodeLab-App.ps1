...
# Get-MenuVmSelection extracted to Private/Get-LabMenuVmSelection.ps1

# Invoke-ConfigureRoleMenu extracted to Private/Invoke-LabConfigureRoleMenu.ps1

# Invoke-AddVMWizard extracted to Private/Invoke-LabAddVMWizard.ps1
# Invoke-AddVMMenu extracted to Private/Invoke-LabAddVMMenu.ps1
...
# Show-Menu extracted to Private/Show-LabMenu.ps1

# Invoke-InteractiveMenu extracted to Private/Invoke-LabInteractiveMenu.ps1
...
$skipLegacyOrchestration = $false
    ...
    if ($orchestrationAction -in @('deploy', 'teardown')) {
        ...
        if ($null -eq $stateProbe) {
            $stateProbe = Resolve-LabRuntimeStateOverride -SkipRuntimeBootstrap:$SkipRuntimeBootstrap
        }
        if ($null -eq $stateProbe) {
    ...
    switch ($Action) {
        'menu' {
            ...
            Invoke-LabInteractiveMenu -LabConfig $GlobalLabConfig -ScriptDir $ScriptDir -SwitchName $SwitchName -LabName $GlobalLabConfig.Lab.Name -EffectiveMode $EffectiveMode -RunEvents $RunEvents -NonInteractive:$NonInteractive -AutoFixSubnetConflict:$AutoFixSubnetConflict
        }
