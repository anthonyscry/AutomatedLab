...
function Get-LabFailureErrorCode {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
...
function Invoke-LabCliCommand {
    ...
    $defaultLogRoot = [System.IO.Path]::GetFullPath((Join-Path -Path $PSScriptRoot -ChildPath '../../artifacts/logs'))
    $commandMap = Get-LabCommandMap

    $resolvedLogRoot = $defaultLogRoot
    $result = $null

    try {
        $resolvedLogRoot = Resolve-LabLogRoot -ConfigPath $ConfigPath -DefaultLogRoot $defaultLogRoot
    ...
    if ($null -eq $result) {
        if (-not $commandMap.Contains($Command)) {
            ...
            try {
                ...
                $result.ErrorCode = Get-LabFailureErrorCode -FailureCategory $result.FailureCategory
                $result.RecoveryHint = $_.Exception.Message
            }
        }
    }
