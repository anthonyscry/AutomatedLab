...
"@
    ...
    2. `New-CidataVhdx` lives in `Public/Linux/New-CidataVhdx.ps1` (NOT Lab-Common.ps1) and already supports both Ubuntu and Rocky Linux 9 (RHEL-family) via NoCloud datasource. No changes needed to `New-CidataVhdx` — CentOS Stream 9 uses the same cloud-init NoCloud format. The `Invoke-LinuxRoleCreateVM` `-ISOPattern` parameter is the only differentiator (Ubuntu vs CentOS ISO filename).
