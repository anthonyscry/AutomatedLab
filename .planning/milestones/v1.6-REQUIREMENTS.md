# Requirements Archive: v1.6 Lab Lifecycle & Security Automation
...
- [x] **TTL-01**: Operator can configure lab TTL duration in Lab-Config.ps1 (hours, with safe defaults)
- [x] **TTL-02**: Background scheduled task auto-suspends all lab VMs when TTL expires
- [x] **TTL-03**: Lab uptime is tracked and queryable via Get-LabUptime cmdlet
...
- [x] **DASH-01**: Per-VM snapshot age displayed on dashboard with configurable staleness warnings
- [x] **DASH-02**: VHDx disk usage shown per VM with disk pressure indicators
- [x] **DASH-03**: VM uptime displayed with configurable stale threshold alerts
- [x] **DASH-04**: STIG compliance status column reads from cached compliance JSON data
- [x] **DASH-05**: Background runspace collects enriched VM data without freezing UI thread
...
|-------------|-------|--------|
| TTL-01 | Phase 26 | Complete |
| TTL-02 | Phase 26 | Complete |
| TTL-03 | Phase 26 | Complete |
...
*Last updated: 2026-02-21 after v1.6 completion (18/18 complete)*
